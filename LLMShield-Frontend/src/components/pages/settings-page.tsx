...
export function SettingsPage() {
  ...
  const [settings, setSettings] = useState({
    ...
    // Scan Settings
    defaultScanTimeout: parseInt(localStorage.getItem('default_scan_timeout_sec') || '300', 10),
  ...
  const saveSettings = () => {
    ...
    // Persist default scan timeout to localStorage for scanner requests
    const sec = Number(settings.defaultScanTimeout)
    const clamped = Number.isFinite(sec) ? Math.min(Math.max(sec, 30), 900) : 300
    localStorage.setItem('default_scan_timeout_sec', String(clamped))
    
    // Show success feedback
    alert(`Settings saved successfully! Default scan timeout set to ${clamped} seconds.`)
  }

  const resetSettings = () => {
    console.log("Resetting settings to defaults")
    // Reset relevant scanning defaults and persist
    const defaults = { defaultScanTimeout: 300, maxConcurrentScans: 3, autoScanEnabled: false, retentionDays: 90 }
    setSettings(prev => ({ ...prev, ...defaults }))
    localStorage.setItem('default_scan_timeout_sec', String(defaults.defaultScanTimeout))
  }

...
export const MainDashboard = memo(() => {
  ...
  const [showMfaPrompt, setShowMfaPrompt] = useState(() => {
    // Only show MFA prompt if user hasn't dismissed it and MFA is not enabled
    const hasSeenWelcome = sessionStorage.getItem('welcomePopupShown');
    const hasDismissedMfaPrompt = sessionStorage.getItem('mfaPromptDismissed');
    return hasSeenWelcome && !hasDismissedMfaPrompt && !mfaStatus?.enabled;
  });

  // Update MFA prompt visibility when welcome popup closes or MFA status changes
  useEffect(() => {
    if (!showWelcomePopup && !sessionStorage.getItem('mfaPromptDismissed') && !mfaStatus?.enabled) {
  ...
  return (
    ...
    <div className="p-8 scroll-container min-h-screen" style={{backgroundColor: '#1d2736'}}>
      ...
      <MfaPromptPopup
        isOpen={Boolean(showMfaPrompt && !showWelcomePopup)} // Only show after welcome popup is closed
        onClose={handleCloseMfaPrompt}

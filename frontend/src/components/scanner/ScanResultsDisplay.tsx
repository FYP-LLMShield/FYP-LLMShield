...
export const ScanResultsDisplay: React.FC<ScanResultsDisplayProps> = ({ scanResults, onDownloadPDF }) => {
  ...
  React.useEffect(() => {
    if (scanResults.most_affected_files && scanResults.most_affected_files.length > 0) {
      ...
      const fileMap = new Map<string, string>(); // Maps line numbers to filenames
      const filesByLineCount = new Map<string, number>(); // Tracks frequency of each filename
      const lineToFileMap = new Map<number, string>(); // Maps line numbers to filenames more directly
      ...
      if (scanResults.critical_findings && scanResults.critical_findings.length > 0) {
        scanResults.critical_findings.forEach((finding: any) => {
      ...
      if (scanResults.findings && scanResults.findings.length > 0) {
        scanResults.findings.forEach((finding: any) => {
          if (finding.file && finding.line) {
            fileMap.set(finding.line.toString(), finding.file);
            lineToFileMap.set(finding.line, finding.file);
      ...
      const updatedFiles = scanResults.most_affected_files.map((file: any) => {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="col-span-1 md:col-span-2">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                ...
                <div className="text-center p-3 bg-gray-100 dark:bg-gray-800 rounded-lg">
                  <div className="text-3xl font-bold">
                    {new Set(scanResults.findings.map((f: any) => f.file)).size}
                  </div>
              ...
              <ul className="list-none pl-2 space-y-2">
                {scanResults.executive_summary.top_risks.map((risk: any, index: number) => {
      ...
      {(scanResults.critical_findings?.length ?? 0) > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-4">
              {(scanResults.critical_findings ?? []).map((finding: any) => (
      ...
      <Tabs defaultValue="findings">
            ...
            <CardContent>
              <div className="space-y-4">
                {scanResults.most_affected_files && scanResults.most_affected_files.length > 0 ? (
                  scanResults.most_affected_files.map((file: any, index: number) => (
        ...
        <TabsContent value="next-steps">
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {(scanResults.next_steps ?? []).map((step: any, index: number) => (

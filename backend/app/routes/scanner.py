...
def scan_text_content(content: str, filename: str, scan_types: List[str]) -> List[FindingModel]:
    ...
    try:
        ...
        # DEEP COMPREHENSIVE GEMINI ANALYSIS - Always run for thorough security assessment (Abeeha: C/C++)
        try:
...
def create_enhanced_response(
    ...
    print(f"Category counts - Secrets: {category_breakdown.secrets}, C++ Vulns: {category_breakdown.cpp_vulnerabilities}")
...
def generate_pdf_report(scan_response: ScanResponse) -> bytes:
    ...
    # Make sure both categories are included in the chart
    bc.data = [[cat_break.secrets or 0, cat_break.cpp_vulnerabilities or 0]]

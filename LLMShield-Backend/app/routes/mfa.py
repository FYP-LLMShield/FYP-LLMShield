...
async def disable_mfa(
    ...
    try:
        ...
        # Verify password
        if not await user_service.verify_password(email, disable_request.current_password):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        # Disable MFA
        success = await user_service.disable_mfa(email)
...
async def regenerate_recovery_codes(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        success = await user_service.update_recovery_codes(email, new_recovery_codes)
